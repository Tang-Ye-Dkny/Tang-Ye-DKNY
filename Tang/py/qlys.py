...
class Spider(Spider):
    ...
    def detailContent(self, ids):
        ...
        # --- 解析多線路核心邏輯修改 ---
        try:
            # 1. 解析播放線路標籤名稱與對應 data-target/id (如 playlist-1 -> WSY, playlist-3 -> 西瓜)
            tabs_map = {}
            tabs_order = []
            for tab_m in re.finditer(r'<span[^>]*data-target="(playlist-\d+)"[^>]*>\s*(.*?)\s*</span>', h, re.S):
                target_id = tab_m.group(1)
                source_name = re.sub(r'<[^>]+>', '', tab_m.group(2)).strip()
                if target_id and source_name:
                    tabs_map[target_id] = source_name
                    tabs_order.append(target_id)

            # 2. 解析各播放列表內容區塊
            play_from_list = []
            play_url_list = []

            for block_m in re.finditer(r'<div\s+id="(playlist-\d+)"[^>]*class="source-pane[^"]*"[^>]*>(.*?)</div>\s*</div>', h, re.S):
                pid = block_m.group(1)
                block_html = block_m.group(2)
                
                # 提取該線路下所有劇集
                eps = re.findall(r'href="(/index\.php/vod/play/id/[^"]+/sid/\d+/nid/\d+\.html)"[^>]*>([^<]*)<', block_html)
                el = []
                for u, n in eps:
                    n = n.strip()
                ...
                if el:
                    s_name = tabs_map.get(pid, pid)
                    play_from_list.append(s_name)
                    play_url_list.append("#".join(el))

            # 3. 如果未能通過 source-pane 解析，回退至通用 sid 正則匹配
            if not play_from_list:
                ...
                for sid, el in sources.items():
                    s_name = tabs_map.get(f"playlist-{sid}", f"线路{sid}")
                    play_from_list.append(s_name)
                    play_url_list.append("#".join(el))
