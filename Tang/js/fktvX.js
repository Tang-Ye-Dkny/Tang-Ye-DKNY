...
async function detail(id) {
    // 🎯 徹底修復：精確抽取出純字串 ID，防止 Array 物件干擾
    const vodId = Array.isArray(id) ? id[0] : id;
    const res = await req(`${host}${vodId}`, { headers });
    ...
    playList.forEach((it) => {
        ...
        // 🎯 徹底修復：這裡必須 100% 傳入純字串 vodId，絕對不能傳入原來的 id 陣列！
        const urls = indexList.map(idx => {
            const epName = pdfh(idx, 'span.number&&Text').trim();
            const epId = pdfh(idx, 'div&&data-id');
