...
// ========== extractLinesAndEpisodes（凡客专用，纯手写正则） ==========
function extractLinesAndEpisodes(html, vodId) {
    // 1. 提取线路名（data-line 属性）
    let lines = [];
    ...
    if (episodes.length === 0) {
        const altEpRegex = /<a[^>]*data-id="([^"]+)"[^>]*>[\s\S]*?<span[^>]*class="[^"]*number[^"]*"[^>]*>([^<]+)<\/span>/g;
    ...
    for (let line of lines) {
        ...
        const epStr = episodes.map(ep => `${ep.name}$${line.id}-${vodId}-${ep.id}`).join('#');
        playUrls.push(epStr);
    }
    // 兜底
    if (playFroms.length === 0 && episodes.length > 0) {
        playFroms.push('默认线路');
        const epStr = episodes.map(ep => `${ep.name}$${vodId}-${ep.id}`).join('#');
        playUrls.push(epStr);
    }
...
async function detail(id) {
    ...
    try {
        ...
        let vod_pic = "";
        const picMatch = html.match(/class="thumb[^>]*data-src="([^"]+)"/);
        if (picMatch) vod_pic = getProxyUrl(picMatch[1]);
        if (!vod_pic) vod_pic = `https://via.placeholder.com/300x450/333/fff?text=${encodeURIComponent(vod_name)}`;

        let vod_content = "";
        const contentMatch = html.match(/<div class="desc[^>]*>([\s\S]*?)<\/div>/);
        if (contentMatch) vod_content = contentMatch[1].replace(/<[^>]+>/g, '').trim();

        // --- 额外提取类型、年份、演员等（根据需要补充）---
        let vod_type = "";
        const typeMatch = html.match(/类型：<a[^>]*>([^<]+)<\/a>/);
        if (typeMatch) vod_type = typeMatch[1].trim();
        
        let vod_year = "";
        const yearMatch = html.match(/年份：<a[^>]*>([^<]+)<\/a>/);
        if (yearMatch) vod_year = yearMatch[1].trim();
        
        let vod_actor = "";
        const actorMatch = html.match(/主演：<a[^>]*>([^<]+)<\/a>/);
